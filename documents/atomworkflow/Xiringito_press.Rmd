---
title: "Open academic writing: An Atom-based workflow for social scientists"
...
date: "October 17th, 2018 - SocArxiv Symposium"
output:
...
- Then copy the following in the termil, replacin FILENAME for the name of your markdown file:
  - rmarkdown::render('Xiringito_press.Rmd', 'xaringan::moon_reader')
...
bib <- ReadBib("exampleworkflow/Link to MyLibrary.bib", check = FALSE)
```

# Open academic writing?

- Academic writing and open science

???
- There is lot of discussion in open science regarding access and reproducibility, this is, focused in how we interact with the wider academic community. However, there is less discussion about how our day-to-day academic practices -as writing - are related to open science.

--

- Components of the academic writing workflow:
...
- But, what do I mean by writing? This is certainly not only typing, but involves another tools and processes (building blocks)
...
- Is it the most efficient form of version control and collaboration?
...
![](https://kieranhealy.org/files/misc/workflow-wide-tx.png)
...
- based on Emacs ... not very friendly
...
![:scale 50%](images/chiringuito.jpg)
...
- File -> new file (ctrl+n): new document ... start writing.

- File -> save (ctrl+s): save the file. Use the extension .md, so that when open the file will be recognized as markdown document.
...
.right-column[
**Packages:**
- _wordcount_ (a running wordcount in the bottom)
- _language-markdown_ (for syntax highlighting),
...
**Other recommended writing options:**
- enable soft-wrap from core settings
- spell-check package: works in different languages at the same time (!)
- zen package: total content focus
- theme-switcher: switches between themes, ideal for dark/light themes
{{content}}
...
.right-column[
**1.** Add your references collection to Zotero (... in case you are using other program)

  - download / install Zotero
  - export your references from your current reference manager into a standard format (ej: bibtex)
  - in Zotero: File > Import > search for the file.
  - Done
{{content}}
]
...
**2.** Generate an automated, self-synced BibTex collection from Zotero via **Better BibTex** in Zotero:
...
.right-column[
3. Connecting Bib file to markdown

Add the following at the beginning of your markdown document (called a YAML header):
...
Including cites in markdown works indicating the "citation keys" (reference ID):

```
@article{sabbagh_dimension_2003,
  title = {The Dimension of ...
.
.
```

With this we can cite like this:

![:scale 110%](images/citingwithkeys.png)
]

???
of the item in the Bib file, sort of reference ID (generated automatically by the conversion through BetterBibTex.)
...
**Automate with Atom's Zotero-Picker**

- go to Packages > Settings Views > Install Packages
- Search and install Zotero Picker
- Zotero must be open while citing
- Open picker (Packages menu or Alt+Z), is the same Zotero plug-in for Word.
